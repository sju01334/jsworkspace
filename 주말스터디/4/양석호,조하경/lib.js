...
//오브젝트 제거
function removeObject(container,child){
    //부모 div로 부터 제거
    container.removeChild(child);
}


function hitTest(me, target,plusX,plusY) {
    //두물체간 충돌 여부 판단 
    me_x= parseInt(me.style.left);
    me_y= parseInt(me.style.top);
    me_width=parseInt(me.style.width);
    me_height=parseInt(me.style.height);
    target_x= parseInt(target.style.left)+plusX;
    target_y= parseInt(target.style.top)+plusY;
